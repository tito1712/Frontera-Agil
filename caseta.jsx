        ...
        <div className="row" style={{ gap: 14 }}>
          <div className="row" style={{ gap: 11 }}>
            ...
            <div className="stack" style={{ lineHeight: 1.05 }}>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.02rem', color: '#fff' }}>Frontera Ágil</span>
              <span style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--cmuted)' }}>Consola de Caseta</span>
            </div>
          </div>
          <div style={{ width: 1, height: 28, background: 'var(--cline)' }} />
          <div className="row cmuted" style={{ gap: 8, fontSize: '0.84rem', fontWeight: 600 }}><Icon name="pin" size={16} /> {FUNCIONARIO.caseta} · {FUNCIONARIO.paso}</div>
        </div>
        <div className="row" style={{ gap: 16 }}>
          <div className="row cmuted" style={{ gap: 8, fontSize: '0.88rem', fontWeight: 700 }}><Icon name="clock" size={16} /> <Clock /></div>
          <div className="row" style={{ gap: 9, padding: '6px 6px 6px 13px', border: '1px solid var(--cline)', borderRadius: 999, background: 'oklch(0.2 0.025 262)' }}>
            <div className="stack" style={{ alignItems: 'flex-end', lineHeight: 1.15 }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#fff' }}>{FUNCIONARIO.nombre}</span>
              <span className="mono" style={{ fontSize: '0.66rem', color: 'var(--cmuted)' }}>{rol} · {FUNCIONARIO.codigo}</span>
            </div>
...
function CasetaLogin({ go, setRol }) {
  ...
  return (
    <div className="console screen-in" style={{ display: 'grid', placeItems: 'center', minHeight: '100vh', padding: 40 }}>
      <div className="cpanel cpanel-pad stack" style={{ width: 'min(460px, 100%)', gap: 22, padding: 34 }}>
        <div className="stack" style={{ gap: 10 }}>
          ...
          <p className="cmuted">Selecciona tu organismo. La consola priorizará la información que te compete.</p>
        </div>
        <div className="stack" style={{ gap: 10 }}>
          {ROLES.map(r => (
            <button key={r.id} onClick={() => setSel(r.id)} className="row" style={{ gap: 14, textAlign: 'left', padding: 14, borderRadius: 13, cursor: 'pointer', background: sel === r.id ? 'oklch(0.3 0.04 256)' : 'oklch(0.22 0.025 262)', border: '1.5px solid ' + (sel === r.id ? 'oklch(0.65 0.1 256)' : 'var(--cline)') }}>
              <div style={{ width: 42, height: 42, borderRadius: 11, background: sel === r.id ? 'var(--azul)' : 'oklch(0.3 0.03 262)', color: '#fff', display: 'grid', placeItems: 'center' }}><Icon name={r.icon} size={22} /></div>
              <div className="stack" style={{ gap: 2, flex: 1 }}>
                <strong style={{ color: '#fff' }}>{r.id}</strong>
                <span className="cmuted" style={{ fontSize: '0.82rem' }}>{r.desc}</span>
              </div>
              <div style={{ width: 20, height: 20, borderRadius: '50%', border: '2px solid ' + (sel === r.id ? 'var(--azul)' : 'var(--cline)'), background: sel === r.id ? 'var(--azul)' : 'transparent', display: 'grid', placeItems: 'center' }}>{sel === r.id && <Icon name="check" size={12} style={{ color: '#fff' }} stroke={3} />}</div>
            </button>
          ))}
        </div>
        <div className="field"><label style={{ color: 'var(--cmuted)' }}>Código de funcionario</label><input className="input mono" defaultValue={FUNCIONARIO.codigo} /></div>
